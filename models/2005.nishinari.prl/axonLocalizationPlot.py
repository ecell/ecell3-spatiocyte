...
names = sorted(glob.glob("hislog.PositiveFeedbackAxon.Neurite?.csv"))
nFiles = len(names)
f = open(names[0], 'r')
legendTitles = f.readline().strip().split(",")
cols = len(legendTitles)
legendTitles = ['Neurite 1', 'Neurite 2', 'Axon', 'Neurite 3']
...
title('Kinesin Neurite/Axon Tip Localization')
show()
