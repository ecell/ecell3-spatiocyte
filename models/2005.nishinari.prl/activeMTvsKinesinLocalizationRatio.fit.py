...
#sigmoidal (must be normalized first, max y = 1)
def func(x, n, m, kd):
  return (x**n)*m/(kd+x**n)
...
x = []
y = []
for name in glob.glob('tmp.csv'):
  x.extend((genfromtxt(name, delimiter='\t',skip_header=1 ).T)[0])
  y.extend((genfromtxt(name, delimiter='\t',skip_header=1 ).T)[1])
x = np.array(x)
y = np.array(y)
