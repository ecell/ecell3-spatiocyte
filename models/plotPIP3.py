...
fileNames = ['anio.csv', 'pip2.csv', 'anioPip2.csv']
...
data = genfromtxt(fileNames[nFiles-1], delimiter=',').T
d = numpy.array(data)
for i in range(2):
  plot(data[0], d[i+1]+d[i+3], ls=lines[i], color=colors[nSpecies*2+i], label=legendTitles[i], linewidth=1)
